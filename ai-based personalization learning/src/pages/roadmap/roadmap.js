                ...
                .then((res) => {
                  ...
                  setResources(
                    <div className="res">
                      ...
                      <Markdown>{res.data.content}</Markdown>
                    </div>
                  );
                  setTimeout(() => {
          ...
          <button
            ...
            onClick={() => {
              const query = encodeURIComponent(
                `${resourceParam.subtopic} ${topic} tutorial site:youtube.com OR site:coursera.org OR site:udemy.com OR site:geeksforgeeks.org`
              );
              window.open(`https://www.google.com/search?q=${query}`, "_blank");
            }}
          >
            <FolderSearch size={70} strokeWidth={1} className="icon"/> Browse Online Courses
          </button>
        </div>
      </div>
    );
